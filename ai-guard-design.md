...
7. 上下文截断 (history_policy)
   ...
   - 修改请求体后重新序列化
    |
8. 配额预检查 (Redis)
   - 分钟请求次数: key = ai:rl:min:{user_id}:{unix_seconds/60}
   - 窗口请求次数: key = ai:quota:win:{window_secs}:req:{user_id}:{unix_seconds/window_secs}
   - 窗口 token:   key = ai:quota:win:{window_secs}:tok:{user_id}:{unix_seconds/window_secs}
   - window_secs  = quota_window_secs (服务级) ?? default_quota_window_secs (全局) ?? 86400
...
| 用途 | Key 模式 | TTL |
|------|----------|-----|
| 分钟请求计数 | `ai:rl:min:{user_id}:{unix_seconds/60}` | 120s |
| 窗口请求计数 | `ai:quota:win:{window_secs}:req:{user_id}:{unix_seconds/window_secs}` | `window_secs * 2` |
| 窗口 token 计数 | `ai:quota:win:{window_secs}:tok:{user_id}:{unix_seconds/window_secs}` | `window_secs * 2` |

`window_secs` 解析优先级：服务级 `quota_window_secs` > 全局 `default_quota_window_secs` > 默认 `86400`。
...
<Card size="small" title="AI Guard（防滥用）" style={{ marginBottom: 16 }}>
  ...
  <Form.Item
    ...
    label="意图识别系统提示词"
    tooltip="描述本服务允许的任务范围，LLM 将依此判断用户输入是否相关"
  >
    <TextArea
      rows={5}
